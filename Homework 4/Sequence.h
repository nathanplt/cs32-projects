...
class Sequence
{
  ...
  private:
    ...
    void insertBefore(typename Sequence<ItemType>::Node* p, const ItemType& value);
      // Insert value in a new Node before Node p, incrementing m_size.

    typename Sequence<ItemType>::Node* doErase(typename Sequence<ItemType>::Node* p);
      // Remove the Node p, decrementing m_size.  Return the Node that
      // followed p.

    typename Sequence<ItemType>::Node* nodeAtPos(int pos) const;
